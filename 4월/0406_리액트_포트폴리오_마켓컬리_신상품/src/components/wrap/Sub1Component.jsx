import React from 'react';

export default function Sub1Component(){
    return (
        <main id='sub1'>
            <section id='section1'>
                <div className="container">
                    <div className="gap">
                        <div className="content">
                            <a href="!#">
                                <img src="./images/sub1/0HpXehniZlRXiBeSfEFpPK8rSl35tGLPMUOwNNWK.png" alt="" />
                            </a>
                        </div>
                    </div>
                </div>
            </section>
            <section id='section2'>
                <div className="container">
                    <div className="gap">
                        <div className="title">
                            <h2>신상품</h2>
                        </div>
                        <div className="content">
                            <div className="left">
                                <div className="left-title">
                                    <strong>필터</strong>
                                    <span>
                                        <img src="./images/sub1/icon_format.svg" alt="" />
                                        <em>초기화</em>
                                    </span>
                                </div>
                                <div className="left-content">
                                    {/* 체크박스목록 */}
                                    <div>
                                        <a href="!#" className='category-btn category1'><span>카테고리</span> <img src="./images/sub1/icon_arrow_up.svg" alt="" /></a>
                                        <div className="sub">
                                            <ul>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'과일·견과·쌀'} />
                                                        과일·견과·쌀
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'국·반찬·메인요리'} />
                                                        국·반찬·메인요리
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'면·양념·오일'} />
                                                        면·양념·오일
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'수산·해산·건어물'} />
                                                        수산·해산·건어물
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
        
                                            </ul>
                                        </div>
                                        <a href="!#" className='category-btn category1'><span>브랜드</span> <img src="./images/sub1/icon_arrow_up.svg" alt="" /></a>
                                        <div className="sub">
                                            <ul>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'과일·견과·쌀'} />
                                                        과일·견과·쌀
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'국·반찬·메인요리'} />
                                                        국·반찬·메인요리
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'면·양념·오일'} />
                                                        면·양념·오일
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'수산·해산·건어물'} />
                                                        수산·해산·건어물
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
        
                                            </ul>
                                        </div>
                                        <a href="!#" className='category-btn category1'><span>가격</span> <img src="./images/sub1/icon_arrow_up.svg" alt="" /></a>
                                        <div className="sub">
                                            <ul>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'과일·견과·쌀'} />
                                                        과일·견과·쌀
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'국·반찬·메인요리'} />
                                                        국·반찬·메인요리
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'면·양념·오일'} />
                                                        면·양념·오일
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'수산·해산·건어물'} />
                                                        수산·해산·건어물
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
        
                                            </ul>
                                        </div>
                                        <a href="!#" className='category-btn category1'><span>혜택</span> <img src="./images/sub1/icon_arrow_up.svg" alt="" /></a>
                                        <div className="sub">
                                            <ul>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'과일·견과·쌀'} />
                                                        과일·견과·쌀
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'국·반찬·메인요리'} />
                                                        국·반찬·메인요리
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'면·양념·오일'} />
                                                        면·양념·오일
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'수산·해산·건어물'} />
                                                        수산·해산·건어물
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
        
                                            </ul>
                                        </div>
                                        <a href="!#" className='category-btn category1'><span>유형</span> <img src="./images/sub1/icon_arrow_up.svg" alt="" /></a>
                                        <div className="sub">
                                            <ul>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'과일·견과·쌀'} />
                                                        과일·견과·쌀
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'국·반찬·메인요리'} />
                                                        국·반찬·메인요리
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'면·양념·오일'} />
                                                        면·양념·오일
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'수산·해산·건어물'} />
                                                        수산·해산·건어물
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
                                                <li>
                                                    <label htmlFor="category1Chk1">
                                                        <input type="checkbox" name='category1_chk1' id='category1Chk1' value={'샐러드·간편식'} />
                                                        샐러드·간편식
                                                    </label>
                                                </li>
        
                                            </ul>
                                        </div>

                                    </div>                                    
   
                                </div>
                            </div>
                            <div className="right">
                                <div className="right-title">
                                    <strong>총 215건</strong>
                                    <span>
                                        <a href="!#" className='order-btn'><span>추천순</span><img src="./images/sub1/icon_question.svg" alt="" /></a>
                                        <i>|</i>
                                        <a href="!#" className='order-btn on'>신상품순</a>
                                        <i>|</i>
                                        <a href="!#" className='order-btn'>판매량순</a>
                                        <i>|</i>
                                        <a href="!#" className='order-btn'>혜택순</a>
                                        <i>|</i>
                                        <a href="!#" className='order-btn'>낮은 가격순</a>
                                        <i>|</i>
                                        <a href="!#" className='order-btn'>높은 가격순</a>
                                    </span>
                                </div>
                                <div className="right-content">
                                    <ul>
                                        <li>
                                            <a href="!#">
                                                <div className="img-box">
                                                    <img src="./images/sub1/209ac6eb-3130-4ee0-9d51-f3ad007daa11.jpg" alt="" />
                                                    <span>
                                                        <img src="./images/sub1/icon_cart.svg" alt="" />
                                                    </span>
                                                </div>
                                                <div className="tit-box">
                                                    <ul>
                                                        <li>샛별배송</li>
                                                        <li><strong>[브리오신]</strong> <em>사봉 느와르 큐브 비누 300g</em></li>
                                                        <li>얼룩 제거 위한 손 세탁 전용 비누</li>
                                                        <li><span className='rate-price'>45%</span><span className='panme-price'>16,500</span></li>
                                                        <li><s>30,000원</s></li>
                                                        <li>Kurly Only</li>
                                                    </ul>
                                                </div>
                                            </a>

                                        </li>
                                        <li>
                                            <a href="!#">
                                                <div className="img-box">
                                                    <img src="./images/sub1/209ac6eb-3130-4ee0-9d51-f3ad007daa11.jpg" alt="" />
                                                    <span>
                                                        <img src="./images/sub1/icon_cart.svg" alt="" />
                                                    </span>
                                                </div>
                                                <div className="tit-box">
                                                    <ul>
                                                        <li>샛별배송</li>
                                                        <li><strong>[브리오신]</strong> <em>사봉 느와르 큐브 비누 300g</em></li>
                                                        <li>얼룩 제거 위한 손 세탁 전용 비누</li>
                                                        <li><span className='rate-price'>45%</span><span className='panme-price'>16,500</span></li>
                                                        <li><s>30,000원</s></li>
                                                        <li>Kurly Only</li>
                                                    </ul>
                                                </div>
                                            </a>

                                        </li>
                                        <li>
                                            <a href="!#">
                                                <div className="img-box">
                                                    <img src="./images/sub1/209ac6eb-3130-4ee0-9d51-f3ad007daa11.jpg" alt="" />
                                                    <span>
                                                        <img src="./images/sub1/icon_cart.svg" alt="" />
                                                    </span>
                                                </div>
                                                <div className="tit-box">
                                                    <ul>
                                                        <li>샛별배송</li>
                                                        <li><strong>[브리오신]</strong> <em>사봉 느와르 큐브 비누 300g</em></li>
                                                        <li>얼룩 제거 위한 손 세탁 전용 비누</li>
                                                        <li><span className='rate-price'>45%</span><span className='panme-price'>16,500</span></li>
                                                        <li><s>30,000원</s></li>
                                                        <li>Kurly Only</li>
                                                    </ul>
                                                </div>
                                            </a>

                                        </li>
                                        <li>
                                            <a href="!#">
                                                <div className="img-box">
                                                    <img src="./images/sub1/209ac6eb-3130-4ee0-9d51-f3ad007daa11.jpg" alt="" />
                                                    <span>
                                                        <img src="./images/sub1/icon_cart.svg" alt="" />
                                                    </span>
                                                </div>
                                                <div className="tit-box">
                                                    <ul>
                                                        <li>샛별배송</li>
                                                        <li><strong>[브리오신]</strong> <em>사봉 느와르 큐브 비누 300g</em></li>
                                                        <li>얼룩 제거 위한 손 세탁 전용 비누</li>
                                                        <li><span className='rate-price'>45%</span><span className='panme-price'>16,500</span></li>
                                                        <li><s>30,000원</s></li>
                                                        <li>Kurly Only</li>
                                                    </ul>
                                                </div>
                                            </a>

                                        </li>
                                        <li>
                                            <a href="!#">
                                                <div className="img-box">
                                                    <img src="./images/sub1/209ac6eb-3130-4ee0-9d51-f3ad007daa11.jpg" alt="" />
                                                    <span>
                                                        <img src="./images/sub1/icon_cart.svg" alt="" />
                                                    </span>
                                                </div>
                                                <div className="tit-box">
                                                    <ul>
                                                        <li>샛별배송</li>
                                                        <li><strong>[브리오신]</strong> <em>사봉 느와르 큐브 비누 300g</em></li>
                                                        <li>얼룩 제거 위한 손 세탁 전용 비누</li>
                                                        <li><span className='rate-price'>45%</span><span className='panme-price'>16,500</span></li>
                                                        <li><s>30,000원</s></li>
                                                        <li>Kurly Only</li>
                                                    </ul>
                                                </div>
                                            </a>

                                        </li>
                                        <li>
                                            <a href="!#">
                                                <div className="img-box">
                                                    <img src="./images/sub1/209ac6eb-3130-4ee0-9d51-f3ad007daa11.jpg" alt="" />
                                                    <span>
                                                        <img src="./images/sub1/icon_cart.svg" alt="" />
                                                    </span>
                                                </div>
                                                <div className="tit-box">
                                                    <ul>
                                                        <li>샛별배송</li>
                                                        <li><strong>[브리오신]</strong> <em>사봉 느와르 큐브 비누 300g</em></li>
                                                        <li>얼룩 제거 위한 손 세탁 전용 비누</li>
                                                        <li><span className='rate-price'>45%</span><span className='panme-price'>16,500</span></li>
                                                        <li><s>30,000원</s></li>
                                                        <li>Kurly Only</li>
                                                    </ul>
                                                </div>
                                            </a>

                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </main> 
    );
};

