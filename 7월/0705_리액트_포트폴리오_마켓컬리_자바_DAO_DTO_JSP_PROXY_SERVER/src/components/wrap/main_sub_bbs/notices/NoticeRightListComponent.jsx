                        ...
                        </dt>
                        {
                            notice.map((item, idx)=>{
                                if(Math.ceil((idx+1)/list)===pageNumber){    // 클릭버튼 번호 전달
                                    return(                                    
                                        <dd key={item.NO} data-key={item.NO}> 
                                            <Link to={`/notice_view?listNum=${item.NO}`}>
                                                <span>{item.NO}</span>
                                                <span>{item.제목}</span>
                                                <span>{item.날짜}</span>
                                                <span>{item.조회수}</span>
                            ...
                            (()=>{
                                ...
                                for(let i=startNum; i<endNum; i++){
                                    ...
                                    if(i<Math.ceil(notice.length/list)){
                                        arr = [...arr, <a key={i} data-key={`num ${i}`} className={pageNumber===(i+1)?'on':''} href='!#' onClick={(e)=>onClickPageNum(e, (i+1))}>{i+1}</a>]
