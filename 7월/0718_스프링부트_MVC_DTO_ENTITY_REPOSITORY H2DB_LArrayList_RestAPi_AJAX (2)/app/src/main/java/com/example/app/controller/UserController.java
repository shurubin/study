...
@Controller
@Slf4j  // 롬복 로깅 사용 어노테이션(Simple Logging Facade for Java);
public class UserController {
    ...
    // 글목록 ArrayList 목록 출랙
    // 폼데이터(DTO) => 엔티티(Entity) => 리파지토리(DB저장)
    // => 저장된 DB목록 => ArrayList list사용 list.do(목록출력)  뷰 탬플릿(머쉬태시, 타임리프)
    // 엔티티(Entity) => ArrayList list사용 list.do(목록출력) => 뷰 탬플릿(머쉬태시, 타임리프)
