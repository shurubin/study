...
export default function SignUpComponent({회원가입, timer}) {
    ...
    const useRef = React.useRef();  //입력상자 태그요소 휴대폰을 선택함.(참조함)
    let cnt = React.useRef(1000);
    ...
    // useRef 새로고침해야만 증가값이 보인다
    // useRef 새로고침해도 값이 유지된다


    //state 상태변수는 새로고침하면 새로 시작 초기화
    //state 상태변수는 변경되면 화면이 다시 그려진다. 마운트된다
    ...
    const onClickConfirmModal=(e)=>{
        ...
        console.log(state.isConfirmModalMsg.includes('발송인증번호'));
        console.log(state.isConfirmModalMsg.indexOf('발송인증번호'));
        console.log(state.isConfirmModalMsg.search('발송인증번호'));

        if(state.isConfirmModalMsg.includes('발송인증번호')){
            //모달닫기 그리고 함수실행
            //
        }
        else {
    ...
    const onClickHpNum2=(e)=>{
        ...
        //발송인증번호를 String 또는 입력인증번호를 Number
        if(String(state.발송인증번호)!==state.입력인증번호){
            isConfirmModalMsg = "잘못된 입력번호입니다.";
            isHp3 = true;
            발송인증번호= state.발송인증번호;
            isHpDisabled = false;
        }
    ...
    const onClickHpNum3=(e)=>{
        ...
        //1. 맨 위 루트영역어 ref를 생성하고 변수 createRef에 저장한다.
        //2. createRef 변수를 태그요소 휴대편 입력상자에 ref={createRef}에 대입시킨다
        //3. 이벤트 수행 시 휴대폰 입력자에 커서를 포커스 시킨다. createRef.current.focus()
        //휴대폰 입력상자에 커서를 포커스시킨다.
        createRef.current.focus(); //커서를 휴대폰 입력상자에 맞춘다. 커서가 깜빡깜빡
    }
...
SignUpComponent.defaultProps = {
    회원가입: {
        ...
        //6-2 휴대폰 인증번호 받기 버튼 disabled
        isHpDisabled: true, //휴대폰 인증번호 받기버튼 사용불가        
        발송인증번호: '',

        //6-3 휴대폰 인증번호 확인 버튼 disabled
        입력인증번호: '',
        isHpDisabled2: true, //휴대폰 인증번호 확인버튼 사용불가

        //6-4 인증번호 확인 클릭 이벤트

        //6-5 휴대폰 다른번호인증 : 인증번호 성공하면 보임
        isHp3: true,
