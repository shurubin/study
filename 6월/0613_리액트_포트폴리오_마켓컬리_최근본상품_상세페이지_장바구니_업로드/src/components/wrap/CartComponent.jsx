...
export default function CartComponent({openPopupDaumPostApi, addr, confirmModalOpen}) {
    ...
    },[delcode])    // 삭제 상태관리변수가 변경되면 모달창을 띄운다
            ...
            return(
                <dd key={idx}>
                    <ul>
                        ...
                        <li>
                            <span className={`cart-small-bgimg  blind${item.수량===1?' on':''}`} style={{backgroundImage: `url(${item.이미지})`}}>Cart Small Image</span>
                        </li>
                        <li>
                            <a href="!#">{item.제품명}</a>
                        </li>
                        <li>
                            <div>
                                <button onClick={(e)=>onClickSUB(e, item)} className='sub-bgimg-btn blind'>빼기</button>
            ...
            return(
                <dd key={idx} >
                    <ul>
                        ...
                        <li>
                            <span className={`cart-small-bgimg  blind${item.수량===1?' on':''}`} style={{backgroundImage: `url(${item.이미지})` }}></span>
                        </li>
                        <li>
                            <a href="!#">{item.제품명}</a>
                        </li>
