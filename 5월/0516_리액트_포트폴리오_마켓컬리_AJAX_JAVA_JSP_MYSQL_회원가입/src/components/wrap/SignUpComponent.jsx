...
export default function SignUpComponent({회원가입, timer, timerCounterfn, mapAddressFn}) {
    ...
    const onChangeUserService=(e)=>{        
        ...
        if(e.target.checked===true){// 개별체크 선택항목 value 값을 이용약관동의 배열에 저장시킨다.

            // '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' 이 체크되면
            // SNS, 이메일 체크해야한다.
            // 체크된다는 건 이용약관 동의배열에 SNS와 이메일을 저장한다.
            if(e.target.value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' && state.이용약관동의.includes('SNS')===false && state.이용약관동의.includes('이메일')===false){
                ...
                setState({
                    ...state,
                    이용약관동의: [...state.이용약관동의, e.target.value, 'SNS']  // 이메일 체크됨 hook state에 중복되지 않게해야함
                ...
                setState({
                    ...state,
                    이용약관동의: [...state.이용약관동의, e.target.value, '이메일']  // SNS 체크됨 hook state에 중복되지 않게해야함
                ...
                setState({
                    ...state,
                    이용약관동의: [...state.이용약관동의, e.target.value ]// SNS 이메일 둘 다 체크됨 hook state에 중복되지 않게해야함
                    
                })
            }
            // 현재 SNS를 체크했는데 이메일이 체크되어있고, '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' 선택이 안 되어있다면
            else if (e.target.value==='SNS' && state.이용약관동의.includes('무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)')===false && state.이용약관동의.includes('이메일')===true){

                setState({
                    ...state,
                    이용약관동의: [...state.이용약관동의, e.target.value, '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' ]// SNS 이메일 둘 다 체크됨 hook state에 중복되지 않게해야함
            ...
            // 1.'무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' 해제하면 : state.이용약관동의 배열값 3개 모두 삭제
                //      삭제1 : [state. 이용약관동의] : 무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택) : => 임시배열에 저장한다
                //      삭제2 : [이용약관동의] SNS
                //      삭제3 : [이용약관동의] 이메일

            if(e.target.value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)'){

                이용약관동의 = state.이용약관동의.filter((item)=>item!==e.target.value);  //삭제1
                ...
                setState({
                    ...state,
                    이용약관동의: 이용약관동의 // 삭제되고 새롭게 배열 구성완료
                })
    ...
    const onSubmitSignupEvent=(e)=>{
        ...
        // }
        // $.ajax({
        //     url:'https://susubini03.dothome.co.kr/signup_db/insert.php',
        //     type:'POST',  //axios는 method: 'post'
        //     data: formData,
        //     success(res){
        ...
        // 가입하기 버튼 클릭했을 때 검증 조건문
        // 입력폼 화면의 필수 항목과 선택 항목, 그리고 중복확인, 인증등 항목들의
        // 빠짐없는 항목을 체크하고 가용성있는 폼데이터를 전송한다.
        // 1. 아이디 : 빈값이면 입력요구
        // 2. 아이디 중복확인 : 중복확인실행여부를 검사 isIdOk
        // 3. 비밀번호 : 빈값이면 입력 요구
        // 4. 비밀번호확인 : 비밀번호와 동일한지 확인  isPw2 false이고 빈값이 아니면
        // 5. 이름 : 빈값이면 입력 요구
        // 6. 이메일 : 빈값이면 입력요구
        // 7. 이메일 중복확인 : isEmailOk
        // 8. 휴대폰 : 빈값이면 입력 요구
        // 9. 휴대폰 인증 : 인증번호 성공 여부확인   *추가 필요 isHpOk true면 성공
        // 10. 주소1, 주소2 : 빈값이면 입력 요구

        // 11. 성별 : 선택사항이므로  유효성에서 제외
        // 12. 생년월일 : 선택사항이므로  유효성에서 제외
        // 13. 추가입력사항 : 선택사항이므로  유효성에서 제외
        //      가입하기 클릭하면 이용약관동의 배열값 내용 중 필수항목을 카운트한다.
  
        // 14. 이용약관동의 : 필수항목 3개 확인 //*추가 필요
        // 1 ~14번까지 이상없으면 전송
        ...
        if(state.아이디중복확인===false){
        ...
        else if(state.비밀번호확인!==state.비밀번호){
            setState({
                ...state,
                isConfirmModal: true,
                confirmMsg: '비밀번호를 확인해주세요'
            })
        }
        else if(state.이름===''){
            setState({
            ...
            //1. JSP 웹문서 상단 헤더에 CORS 헤더 추가하기
            //2. 서블릿컨테이너(톰켓서버 켜기)

            
            // http://127.0.0.1:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // http://192.168.40.126:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // http://localhost:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // AJAX
            // $.ajax({
            //     // url:'http://192.168.40.126:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action_test_parameter.jsp',
            //     // url:'http://192.168.40.126:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action_test_useBean.jsp',
            //     url:'http://192.168.40.126:8080/jsp/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp',
            //     type:'POST',
            //     data: formData,
            //     success(res){
            //         console.log('AJAX 성공');
            ...
            // [1] AXIOUS() post구현 => 1. url package.json 에서 proxy 설정
        ...
        // indexOf('필수') 찾으면  0 1 2 3... 글자 인덱스 값
        // 찾지 못하면 -1
        ...
        //         cnt++;
        //     }
        //     console.log(`item.indexOf('필수')${item.search('필수')}`);
        // });
