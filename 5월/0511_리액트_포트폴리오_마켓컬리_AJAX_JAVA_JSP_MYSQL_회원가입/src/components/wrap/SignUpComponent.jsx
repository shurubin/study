...
export default function SignUpComponent({회원가입, timer, timerCounterfn, mapAddressFn}) {
    ...
    const onSubmitSignupEvent=(e)=>{
        ...
        if(state.아이디중복확인===false){
            ...
            // Node.js기반에서 리액트가 있는 위치 http://localhost:3000/ => http://susubini03.dothome.co.kr/signup_db/select.php
            // 프로토콜://도메인:포트번호/경로/index.html
            // 브라우저가 보내는 주소(출처)와 받는 주소가 같은지 검사 정책 SOP RFC 6454 보안정책
            // => 출처가 같은 철처에서만 공유할 수 있다
            axios({
                // url : '닷홈웹호스팅_폼데이터를_전송해서_폼의속성을_받는스크립트언어'
                url: 'http://susubini03.dothome.co.kr/signup_db/form_data_insert.php',    // 닷홈 호스팅 서버 : 서버사이드 스크립트(JSP, PHP, ASP) 언어 => SQL => 데이터베이스 저장소
                method:'POST',
                data: newFormData    
            })
            .then((res)=>{
